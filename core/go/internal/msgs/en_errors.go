...
var (
	...
	MsgSequencerParseFailed                        = pde("PD012604", "Failed to parse message")
	MsgSequencerInvalidMessage                     = pde("PD012605", "Invalid message type %s received from transport")
	MsgSequencerKeyResolutionFailed                = pde("PD012606", "Key resolution failed for key %s, algorithm %s, verifierType %s")
	MsgSequencerDeployInitFailed                   = pde("PD012607", "Failed to initialise a deploy transaction")
	MsgSequencerDeployPrepareFailed                = pde("PD012608", "Failed to prepare a deploy transaction")
	MsgSequencerDeployPrepareIncomplete            = pde("PD012609", "Prepare step did not return a transaction to invoke, or a transaction to deploy")
	MsgSequencerBaseLedgerTransactionFailed        = pde("PD012610", "Failed to submit base ledger transaction")
	MsgSequencerContractAddressNotProvided         = pde("PD012611", "Contract address (To) not found in the transaction input")
	MsgSequencerPublicTxFail                       = pde("PD012612", "Public transaction rejected")
	MsgSequencerResolveVerifierRemoteFailed        = pde("PD012613", "Failed to resolve verifier on remote node with lookup %s algorithm %s: Error %s")
	MsgSequencerAssembleRevert                     = pde("PD012614", "Domain reverted transaction on assemble: %s")
	MsgSequencerResolveError                       = pde("PD012615", "Failed to resolve local signer for party %s (verifier=%s,algorithm=%s): %s")
	MsgSequencerSignError                          = pde("PD012616", "Failed to sign for party %s (verifier=%s,algorithm=%s): %s")
	MsgSequencerEndorsementRequestError            = pde("PD012617", "Failed to request endorsement from %s: %s")
	MsgSequencerResolveDispatchError               = pde("PD012618", "Failed to resolve dispatcher: %s")
	MsgSequencerPrepareError                       = pde("PD012619", "Failed to prepare transaction dispatch to base ledger: %s")
	MsgSequencerDeployError                        = pde("PD012620", "Failed to deploy private contract")
	MsgSequencerEncodeCallDataFailed               = pde("PD012621", "Failed to encode call data '%s' for private contract deploy")
	MsgSequencerNonLocalSigningAddr                = pde("PD012622", "Attempt do dispatch a blockchain transaction using a signing identity for a different node: %s")
	MsgSequencerReDelegationRequired               = pde("PD012623", "Re-delegation is required for this transaction to progress")
	MsgSequencerDomainMismatch                     = pde("PD012624", "Domain '%s' specified does not match domain '%s' of deployed private smart contract %s")
	MsgSequencerInvalidPrepareOutcome              = pde("PD012625", "Prepare outcome unexpected for transaction %s intent=%s public-submission=%t private-submission=%t")
	MsgSequencerPrepareNotSupportedDeploy          = pde("PD012626", "Preparing transactions for external submission is not supported for deploy")
	MsgSequencerInvalidEventMissingField           = pde("PD012627", "Invalid event: missing field %s")
	MsgSequencerSignRemoteError                    = pde("PD012628", "Attempt to sign a transaction with an identity from a remote node: %s")
	MsgSequencerFromNotResolvedDistroTime          = pde("PD012629", "Failed to extract node from fully qualified from address for state distribution")
	MsgSequencerInvalidTxStateStateDistro          = pde("PD012630", "Invalid transaction state for state distribution")
	MsgSequencerDistributionNotFullyQualified      = pde("PD012631", "State distribution from domain is not fully qualified: %s")
	MsgSequencerInvalidNullifierSpecInDistro       = pde("PD012632", "Invalid nullifier specification in new state instruction from domain")
	MsgSequencerNewSequencerError                  = pde("PD012633", "Failed to create new sequencer")
	MsgSequencerInvalidStaticCoordinator           = pde("PD012634", "Contract was configured with invalid static coordinator '%s'.  Must be of the form 'identity@node'")
	MsgSequencerFunctionNotProvided                = pde("PD012635", "Function abi not provided in transaction input")
	MsgSequencerAssembleRequestInvalid             = pde("PD012636", "Assemble request is invalid for transaction %s")
	MsgSequencerAssembleTxnNotFound                = pde("PD012637", "Transaction %s not found in local node")
	MsgSequencerMaxInflightTransactions            = pde("PD012638", "Max number of inflight transactions (%d) reached. %d transactions delegated from node %s, %d already being coordinated on this node, %d new transactions accepted for coordination, %d rejected")
	MsgSequencerErrorNotifyingDependent            = pde("PD012639", "Error notifying dependent transaction %s of revert of TX %s")
	MsgSequencerErrorRepoolingTX                   = pde("PD012640", "Error repooling TX %s")
	MsgSequencerTransactionNotFound                = pde("PD012641", "TX %s missing from coordinator")
	MsgSequencerInvalidEndorserCandidate           = pde("PD012642", "Invalid coordinator endorser candidate identity '%s'")
	MsgSequencerGrapherAddMinterAlreadyExistsError = pde("PD012643", "Error adding TX %s as minter for state %s, already minted by %s")
	MsgSequencerLoadSequencerError                 = pde("PD012644", "Failed to load sequencer for contract %s: %s")
	MsgSequencerEndorsementUnmarshalError          = pde("PD012645", "Failed to unmarshal %s for endorsement request: %s")
	MsgSequencerEndorsementPartyResolutionError    = pde("PD012646", "Failed to resolve endorsement party identity %s: %s")
	MsgSequencerEndorseTransactionError            = pde("PD012647", "Failed to endorse transaction: %s")
	MsgSequencerEndorserIdentityError              = pde("PD012648", "Failed to validate endorser identity %s: %s")
	MsgSequencerEndorsementResponseSendError       = pde("PD012649", "Failed to send endorsement response for transaction %s: %s")
	MsgSequencerUnexpectedAssembleResult           = pde("PD012650", "Received unexpected assembly result %s")
	MsgSequencerAssembleParkNotAllowed             = pde("PD012651", "Coordinator received unexpected PARK result during assembly")
	MsgSequencerTransactionIDParseError            = pde("PD012652", "Failed to parse transaction ID %s: %s")
	MsgSequencerProtoUnmarshalError                = pde("PD012653", "Failed to unmarshal proto message %s from %s: %s")
