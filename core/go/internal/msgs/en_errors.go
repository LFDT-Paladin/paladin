...
var (
	...
	// PrivTxMgr PD0118XX
	// Message range no longer used. See distributed sequencer range PD0126XX
	...
	MsgSequencerDomainNotProvided                  = pde("PD012602", "Domain not found in the transaction input")
	MsgSequencerAssembleError                      = pde("PD012603", "Error assembling transaction: %s")
	MsgSequencerInvalidMessage                     = pde("PD012604", "Invalid message type %s received from transport")
	MsgSequencerKeyResolutionFailed                = pde("PD012605", "Key resolution failed for key %s, algorithm %s, verifierType %s")
	MsgSequencerDeployInitFailed                   = pde("PD012606", "Failed to initialise a deploy transaction")
	MsgSequencerDeployPrepareFailed                = pde("PD012607", "Failed to prepare a deploy transaction")
	MsgSequencerDeployPrepareIncomplete            = pde("PD012608", "Prepare step did not return a transaction to invoke, or a transaction to deploy")
	MsgSequencerContractAddressNotProvided         = pde("PD012609", "Contract address (To) not found in the transaction input")
	MsgSequencerResolveVerifierRemoteFailed        = pde("PD012610", "Failed to resolve verifier on remote node with lookup %s algorithm %s: Error %s")
	MsgSequencerAssembleRevert                     = pde("PD012611", "Domain reverted transaction on assemble: %s")
	MsgSequencerResolveError                       = pde("PD012612", "Failed to resolve local signer for party %s (verifier=%s,algorithm=%s): %s")
	MsgSequencerSignError                          = pde("PD012613", "Failed to sign for party %s (verifier=%s,algorithm=%s): %s")
	MsgSequencerEndorsementRequestError            = pde("PD012614", "Failed to request endorsement from %s: %s")
	MsgSequencerResolveDispatchError               = pde("PD012615", "Failed to resolve dispatcher: %s")
	MsgSequencerPrepareError                       = pde("PD012616", "Failed to prepare transaction dispatch to base ledger: %s")
	MsgSequencerDeployError                        = pde("PD012617", "Failed to deploy private contract")
	MsgSequencerEncodeCallDataFailed               = pde("PD012618", "Failed to encode call data '%s' for private contract deploy")
	MsgSequencerNonLocalSigningAddr                = pde("PD012619", "Attempt do dispatch a blockchain transaction using a signing identity for a different node: %s")
	MsgSequencerDomainMismatch                     = pde("PD012620", "Domain '%s' specified does not match domain '%s' of deployed private smart contract %s")
	MsgSequencerInvalidPrepareOutcome              = pde("PD012621", "Prepare outcome unexpected for transaction %s intent=%s public-submission=%t private-submission=%t")
	MsgSequencerPrepareNotSupportedDeploy          = pde("PD012622", "Preparing transactions for external submission is not supported for deploy")
	MsgSequencerFromNotResolvedDistroTime          = pde("PD012623", "Failed to extract node from fully qualified from address for state distribution")
	MsgSequencerInvalidTxStateStateDistro          = pde("PD012624", "Invalid transaction state for state distribution")
	MsgSequencerDistributionNotFullyQualified      = pde("PD012625", "State distribution from domain is not fully qualified: %s")
	MsgSequencerInvalidNullifierSpecInDistro       = pde("PD012626", "Invalid nullifier specification in new state instruction from domain")
	MsgSequencerNewSequencerError                  = pde("PD012627", "Failed to create new sequencer")
	MsgSequencerInvalidStaticCoordinator           = pde("PD012628", "Contract was configured with invalid static coordinator '%s'.  Must be of the form 'identity@node'")
	MsgSequencerFunctionNotProvided                = pde("PD012629", "Function abi not provided in transaction input")
	MsgSequencerAssembleRequestInvalid             = pde("PD012630", "Assemble request is invalid for transaction %s")
	MsgSequencerAssembleTxnNotFound                = pde("PD012631", "Transaction %s not found in local node")
	MsgSequencerMaxInflightTransactions            = pde("PD012632", "Max number of inflight transactions (%d) reached. %d transactions delegated from node %s, %d already being coordinated on this node, %d new transactions accepted for coordination, %d rejected")
	MsgSequencerErrorNotifyingDependent            = pde("PD012633", "Error notifying dependent transaction %s of revert of TX %s")
	MsgSequencerTransactionNotFound                = pde("PD012634", "TX %s missing from coordinator")
	MsgSequencerInvalidEndorserCandidate           = pde("PD012635", "Invalid coordinator endorser candidate identity '%s'")
	MsgSequencerGrapherAddMinterAlreadyExistsError = pde("PD012636", "Error adding TX %s as minter for state %s, already minted by %s")
	MsgSequencerLoadSequencerError                 = pde("PD012637", "Failed to load sequencer for contract %s: %s")
	MsgSequencerEndorsementUnmarshalError          = pde("PD012638", "Failed to unmarshal %s for endorsement request: %s")
	MsgSequencerEndorsementPartyResolutionError    = pde("PD012639", "Failed to resolve endorsement party identity %s: %s")
	MsgSequencerEndorseTransactionError            = pde("PD012640", "Failed to endorse transaction: %s")
	MsgSequencerEndorserIdentityError              = pde("PD012641", "Failed to validate endorser identity %s: %s")
	MsgSequencerEndorsementResponseSendError       = pde("PD012642", "Failed to send endorsement response for transaction %s: %s")
	MsgSequencerUnexpectedAssembleResult           = pde("PD012643", "Received unexpected assembly result %s")
	MsgSequencerAssembleParkNotAllowed             = pde("PD012644", "Coordinator received unexpected PARK result during assembly")
	MsgSequencerTransactionIDParseError            = pde("PD012645", "Failed to parse transaction ID %s: %s")
	MsgSequencerProtoUnmarshalError                = pde("PD012646", "Failed to unmarshal proto message %s from %s: %s")
	MsgSequencerMessageFieldMissing                = pde("PD012647", "Field %s missing from proto message %s received from %s")
	MsgSequencerJSONUnmarshalError                 = pde("PD012648", "Failed to unmarshal JSON object %s from proto message %s received from %s: %s")
	MsgSequencerContractAddressParseError          = pde("PD012649", "Failed to parse contract address from proto message %s received from %s: %s")
	MsgSequencerQueryPendingTxError                = pde("PD012650", "Failed to query pending transactions to resume: %s")
	MsgSequencerResumePendingTxError               = pde("PD012651", "Failed to resume pending transaction %s: %s")
	MsgSequencerDeployEvaluationError              = pde("PD012652", "Failed to evaluate deployment: %s")
	MsgSequencerPersistBatchError                  = pde("PD012653", "Failed to persist dispatch batch: %s")
	MsgSequencerFinalizeDeployError                = pde("PD012654", "Failed to finalize deployment: %s")
	MsgSequencerUnexpectedReceiptType              = pde("PD012655", "Unexpected receipt type %d for transaction %s")
	MsgSequencerGetPublicTxError                   = pde("PD012656", "Failed to get public transaction by ID: %s")
	MsgSequencerHandleConfirmedEventError          = pde("PD012657", "Failed to handle transaction confirmed event: %s")
	MsgSequencerChainedRecordCheckError            = pde("PD012658", "Failed to check chained records for transaction %s: %s")
