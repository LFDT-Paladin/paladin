...
var (
	...
	MsgSequencerStateHashContention                = pde("PD012625", "Contention detected attempting to spend hash %s in multiple transactions")
	MsgSequencerReDelegationRequired               = pde("PD012626", "Re-delegation is required for this transaction to progress")
	MsgSequencerDomainMismatch                     = pde("PD012627", "Domain '%s' specified does not match domain '%s' of deployed private smart contract %s")
	MsgSequencerInvalidPrepareOutcome              = pde("PD012628", "Prepare outcome unexpected for transaction %s intent=%s public-submission=%t private-submission=%t")
	MsgSequencerPrepareNotSupportedDeploy          = pde("PD012629", "Preparing transactions for external submission is not supported for deploy")
	MsgSequencerInvalidEventMissingField           = pde("PD012631", "Invalid event: missing field %s")
	MsgSequencerSignRemoteError                    = pde("PD012632", "Attempt to sign a transaction with an identity from a remote node: %s")
	MsgSequencerFromNotResolvedDistroTime          = pde("PD012633", "Failed to extract node from fully qualified from address for state distribution")
	MsgSequencerInvalidTxStateStateDistro          = pde("PD012634", "Invalid transaction state for state distribution")
	...
	MsgSequencerStateViewFailed                    = pde("PD012655", "Coordinator state view request %s failed: %s")
	MsgSequencerStateViewInvalid                   = pde("PD012656", "Invalid state view request %s: %s")
	MsgSequencerStateViewUnknownSession            = pde("PD012657", "Unknown assemble session %s")
	MsgSequencerStateViewWrongNode                 = pde("PD012658", "Assemble session %s not owned by requesting node")
