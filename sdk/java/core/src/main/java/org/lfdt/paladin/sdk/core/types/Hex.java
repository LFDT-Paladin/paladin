...
final class Hex {
    ...
    /** Strips a leading {@code 0x} / {@code 0X} prefix if present, otherwise returns the input unchanged. */
    ...
    /**
     * Decodes a hex string (with or without a {@code 0x} prefix, any case) into bytes. An empty string
     * (or a bare {@code "0x"}) decodes to an empty array.
     */
    ...
    /**
     * Parses a base-aware integer string the way the Go SDK does for JSON numbers: a {@code 0x}/{@code 0X}
     * prefix (optionally signed) is read as hex, anything else as decimal.
     */
    static BigInteger parseBigInteger(final String s) {
        ...
        try {
            final BigInteger value = (t.length() >= 2 && t.charAt(0) == '0' && (t.charAt(1) == 'x' || t.charAt(1) == 'X'))
