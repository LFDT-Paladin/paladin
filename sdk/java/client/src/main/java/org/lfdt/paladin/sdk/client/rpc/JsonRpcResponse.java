 ...
 *
 * <p>Immutable. Exactly one of {@link #result()} or {@link #error()} is meaningful for a well-formed
 * response; use {@link #hasError()} to discriminate. The raw {@link #result()} node is left undecoded so the
 * transport can bind it to the caller's requested result type. The {@code id} is kept as a raw node because a
 * node may echo it back as either a string or a number.
 */
