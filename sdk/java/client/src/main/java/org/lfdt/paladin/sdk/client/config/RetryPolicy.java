 ...
 *
 * <p>Immutable; build with {@link #builder()} or take the SDK defaults via {@link #defaults()}. Only
 * transient failures (connection errors, timeouts, and transient HTTP statuses such as {@code 429}/{@code
 * 5xx}) are retried — JSON-RPC application errors and other {@code 4xx} responses are not. The delay before
 * the retry that follows a given failed attempt is {@code initialDelay * factor^(attempt-1)}, capped at
 * {@code maxDelay}; see {@link #delayForAttempt(int)}.
 *
 * <p>Defaults mirror Go's {@code GenericRetryDefaults}/{@code DefaultHTTPConfig}: initial delay 250ms,
 * max delay 30s, factor 2.0, with a bounded {@code maxAttempts} of 5.
 */
public final class RetryPolicy {
    ...
    /**
     * Returns the backoff delay to wait before retrying, given the 1-based number of the attempt that just
     * failed. The first failure ({@code failedAttempt == 1}) waits {@link #initialDelay()}; each subsequent
     * failure multiplies by {@link #factor()}, capped at {@link #maxDelay()}. Mirrors Go's
    ...
    public static final class Builder {
        ...
        /**
         * Sets the maximum number of attempts including the initial try ({@code <= 1} disables retries).
         *
