...
/**
 * Connection configuration for the HTTP JSON-RPC transport, mirroring Go's {@code
 * pldconf.HTTPClientConfig}.
 *
 * <p>Immutable; build with {@link #builder(String)}. The node URL is required; the connect and
 * request timeouts, extra HTTP headers, and {@link RetryPolicy} all default to sensible values
 * (timeouts 30s each, matching Go's {@code DefaultHTTPConfig}). Supplied headers are sent on every
 * request — use them for authentication (e.g. a bearer token) or routing metadata.
 */
public final class RpcClientConfig {
  ...
  public static final class Builder {
    ...
    /**
     * Adds a single HTTP header sent on every request, overwriting any previous value for that
     * name.
     *
     ...
     *
     * @return the configured client config
     */
