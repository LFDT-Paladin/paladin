...
import { getReceiptListener } from "../queries/transactions";
import { ReceiptListenerActions } from "../components/ReceiptListenerActions";

export const ReceiptListenerEntry: React.FC = () => {
  ...
  const { data: listener, error, refetch } = useQuery({
    queryKey: ['receipt-listener', id],
    queryFn: () => getReceiptListener(id!),
      ...
      >
        <Box sx={{ marginBottom: '20px' }}>
          <Button
            ...
            onClick={() => navigate(AppRoutes.ReceiptListeners)}
          >
            {t('backToListeners')}
          </Button>
        </Box>
        <Typography variant="h6" sx={{ marginBottom: '15px' }}>{t('receiptListener')}</Typography>
        <Tabs value="contract"
        ...
        <Box sx={{
          ...
          <ReceiptListenerActions
            ...
            refetch={refetch}
            deleteRefetch={() => navigate(AppRoutes.ReceiptListeners)}
          />
        </Box>
