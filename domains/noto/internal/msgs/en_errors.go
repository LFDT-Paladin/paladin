...
var (
	...
	MsgNullifierContractRequired  = pde("PD200047", "A nullifier can only be derived for a specific contract")
	MsgNullifierWrongContract     = pde("PD200048", "Assembled coin state %d has a nullifier spec for the wrong contract: expected='%s' found='%s'")
	MsgInvalidParams              = pde("PD200049", "Invalid function parameters")
	MsgInvalidSignature           = pde("PD200050", "Signature for '%s' is invalid")
	MsgInvalidTransactionSpec     = pde("PD200051", "Invalid %s in transaction specification")
	MsgInvalidTransactionData     = pde("PD200052", "Invalid %s in transaction data")
	MsgInvalidStoredLockState     = pde("PD200053", "Stored lock state %s is invalid")
	MsgInvalidStoredStateData     = pde("PD200054", "Stored state data %s is invalid: %s")
)
