...
func TestCreateMintLock(t *testing.T) {
	...
	expectedSpendHash, err := n.unlockHashFromIDs_V1(ctx, ethtypes.MustNewAddress(contractAddress), lockID, lockInfo.SpendTxId.HexString(), []string{}, n.endorsableStateIDs(ctx, infoStates[1:2], false), unlockTxData)
	require.NoError(t, err)
