...
func validateTransactionCommon[T ParamValidator](
	...
	if err != nil {
		...
		// invalid values in their transaction delegation, so the error is a revert.
		return nil, *new(T), endorseOnlyRevert(err)
	}
	...
	if abiFn == nil || handler == unsetT {
		return nil, unsetT, revert(i18n.NewError(ctx, msgs.MsgUnknownFunction, functionABI.Name))
	}
	...
	if !exactSignatureMatch {
		return nil, *new(T), revert(i18n.NewError(ctx, msgs.MsgUnexpectedFunctionSignature, functionABI.Name, abiFn.SolString(), tx.FunctionSignature))
	}
...
func (n *Noto) parseCoinList(ctx context.Context, label string, states []*prototk.EndorsableState) (*parsedCoins, NotoDomainError) {
	...
	for i, state := range states {
		if statesUsed[state.Id] {
			return nil, revert(i18n.NewError(ctx, msgs.MsgDuplicateStateInList, label, i, state.Id))
		}
		...
		case n.coinSchema.Id:
			...
			if err != nil {
				return nil, revert(i18n.NewError(ctx, msgs.MsgInvalidListInput, label, i, state.Id, err))
			}
		...
		default:
			return nil, revert(i18n.NewError(ctx, msgs.MsgUnexpectedSchema, state.SchemaId))
		}
	}
	return result, nil
}
...
func (n *Noto) encodeTransactionDataV0(ctx context.Context, transaction *prototk.TransactionSpecification, infoStates []*prototk.EndorsableState) (pldtypes.HexBytes, NotoDomainError) {
	...
	// endorseOnlyRevert, in case this function is ever used on the endorsement path in future,
	...
	for i, state := range infoStates {
		...
		if err != nil {
			return nil, endorseOnlyRevert(err)
		}
	}
	...
	if err != nil {
		// The values encoded here are already-parsed fixed-width types, so a failure is a
...
func (n *Noto) computeLockId(ctx context.Context, contractAddress *pldtypes.EthAddress, notaryAddress *pldtypes.EthAddress, txId string) (pldtypes.Bytes32, NotoDomainError) {
	...
	if err != nil {
		...
		// supplied, so a value that will not encode is a revert.
		return pldtypes.Bytes32{}, endorseOnlyRevert(err)
	}
