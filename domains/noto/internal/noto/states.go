...
func (n *Noto) prepareInputs(ctx context.Context, stateQueryContext string, owner *identityPair, amount *pldtypes.HexUint256, useNullifiers bool) (*preparedInputs, AssembleError) {
	...
	for total.Cmp(amount.Int()) < 0 {
		...
		if len(states) == 0 {
			return nil, assembleError{i18n.NewError(ctx, msgs.MsgInsufficientFunds, total.Text(10)), true}
		}
		for _, state := range states {
			lastStateTimestamp = state.CreatedAt
			coin, err := n.unmarshalCoin(state.DataJson)
			if err != nil {
				return nil, assembleError{i18n.NewError(ctx, msgs.MsgInvalidStateData, state.Id, err), false}
			}
...
func (n *Noto) prepareLockedInputs(ctx context.Context, stateQueryContext string, lockID pldtypes.Bytes32, owner *pldtypes.EthAddress, amount *big.Int, selectAll bool) (*preparedLockedInputs, AssembleError) {
	var lastStateTimestamp int64
	total := big.NewInt(0)
	stateRefs := []*prototk.StateRef{}
	coins := []*types.NotoLockedCoin{}
...
func (n *Noto) prepareLockedOutputs(id pldtypes.Bytes32, owner *identityPair, amount *pldtypes.HexUint256, distributionList identityList) (*preparedLockedOutputs, AssembleError) {
...
func (n *Noto) prepareDataInfo(ctx context.Context, data pldtypes.HexBytes, variant pldtypes.HexUint64, distributionList []string, transaction *prototk.TransactionSpecification, verifiers []*prototk.ResolvedVerifier) ([]*prototk.NewState, AssembleError) {
...
func (n *Noto) prepareLockInfo_V0(lockID pldtypes.Bytes32, owner, delegate *pldtypes.EthAddress, distributionList identityList) (*preparedLockInfo, AssembleError) {
	if delegate == nil {
...
func (n *Noto) prepareLockInfo_V1(newLockInfo *types.NotoLockInfo_V1, distributionList identityList) (*preparedLockInfo, AssembleError) {
...
func (n *Noto) encodeTransferUnmasked(ctx context.Context, contract *ethtypes.Address0xHex, inputs, outputs []*types.NotoCoin) (ethtypes.HexBytes0xPrefix, AssembleOrEndorseError) {
	...
	if err != nil {
		return nil, assembleOrEndorseError{err, false, false}
	}
	return encoded, nil
}

func (n *Noto) encodeLock(ctx context.Context, contract *ethtypes.Address0xHex, inputs, outputs []*types.NotoCoin, lockedOutputs []*types.NotoLockedCoin) (ethtypes.HexBytes0xPrefix, AssembleOrEndorseError) {
	encoded, err := eip712.EncodeTypedDataV4(ctx, &eip712.TypedData{
...
func (n *Noto) encodeUnlock(ctx context.Context, contract *ethtypes.Address0xHex, lockedInputs, lockedOutputs []*types.NotoLockedCoin, outputs []*types.NotoCoin) (ethtypes.HexBytes0xPrefix, AssembleOrEndorseError) {
	encoded, err := eip712.EncodeTypedDataV4(ctx, &eip712.TypedData{
...
func (n *Noto) encodeDelegateLock(ctx context.Context, contract *ethtypes.Address0xHex, lockID pldtypes.Bytes32, delegate *pldtypes.EthAddress, data pldtypes.HexBytes) (ethtypes.HexBytes0xPrefix, AssembleOrEndorseError) {
	encoded, err := eip712.EncodeTypedDataV4(ctx, &eip712.TypedData{
...
func (n *Noto) allocateStateIDs(ctx context.Context, stateQueryContext string, stateLists ...[]*prototk.NewState) AssembleError {
	...
	if err != nil {
		return assembleError{err, false}
	}
