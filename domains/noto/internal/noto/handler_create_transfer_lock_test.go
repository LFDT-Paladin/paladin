...
func TestCreateTransferLock(t *testing.T) {
	...
	expectedSpendHash, err := n.unlockHashFromIDs_V1(ctx, ethtypes.MustNewAddress(contractAddress), lockID, notoOptions.SpendTxId.HexString(), endorsableStateIDs(outputStates[1:2], false), endorsableStateIDs(infoStates[1:2], false), emptyTxData)
	require.NoError(t, err)
	require.Equal(t, expectedSpendHash, fnParams.Params.SpendHash)
	expectedCancelHash, err := n.unlockHashFromIDs_V1(ctx, ethtypes.MustNewAddress(contractAddress), lockID, notoOptions.SpendTxId.HexString(), endorsableStateIDs(outputStates[1:2], false), endorsableStateIDs(infoStates[2:3], false), emptyTxData)
	require.NoError(t, err)
