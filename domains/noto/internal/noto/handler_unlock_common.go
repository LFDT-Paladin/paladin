...
func (h *unlockCommon) validateParams(ctx context.Context, unlockParams *types.UnlockParams) NotoDomainError {
	if unlockParams.LockID.IsZero() {
		return revert(i18n.NewError(ctx, msgs.MsgParameterRequired, "lockId"))
	}
	if len(unlockParams.From) == 0 {
		return revert(i18n.NewError(ctx, msgs.MsgParameterRequired, "from"))
	}
	if len(unlockParams.Recipients) == 0 {
		return revert(i18n.NewError(ctx, msgs.MsgParameterRequired, "recipients"))
	}
	for _, entry := range unlockParams.Recipients {
		if entry.Amount == nil || entry.Amount.Int().Sign() != 1 {
			return revert(i18n.NewError(ctx, msgs.MsgParameterGreaterThanZero, "recipient amount"))
		}
	}
	return nil
}
