...
func (h *lockCommon) checkAllowed(ctx context.Context, tx *types.ParsedTransaction, from string) EndorseError {
	...
	if nodeErr != nil {
		return endorseError{nodeErr, false}
	}
	...
	if tx.Transaction.From != fromQualified.String() {
		return endorseError{i18n.NewError(ctx, msgs.MsgUnlockOnlyCreator, tx.Transaction.From, from), true}
	}
	return nil
}
...
func (h *lockCommon) buildUnlockOperationData(
	...
	infoDistribution identityList,
	infoStates []*prototk.NewState,
) (encodedData []byte, manifestState *prototk.NewState, err AssembleError) {
...
func (h *lockCommon) buildUnlockInfo(ctx context.Context, tx *types.ParsedTransaction, resolvedVerifiers []*prototk.ResolvedVerifier, stateQueryContext string, in *unlockInfoInput) (*unlockInfo, AssembleError) {
...
func (h *lockCommon) getAllRecipientsDistribution(ctx context.Context, notaryID, senderID, fromID *identityPair, recipients []*types.UnlockRecipient, resolvedVerifiers []*prototk.ResolvedVerifier) (identityList, AssembleError) {
...
func (h *lockCommon) assembleUnlockOutputs_V1(ctx context.Context, tx *types.ParsedTransaction, notaryID, fromID *identityPair, recipients []*types.UnlockRecipient, resolvedVerifiers []*prototk.ResolvedVerifier, remainder *big.Int) (*preparedOutputs, AssembleError) {
...
func (h *unlockCommon) validateParams(ctx context.Context, unlockParams *types.UnlockParams) AssembleOrEndorseError {
	if unlockParams.LockID.IsZero() {
		return assembleOrEndorseError{i18n.NewError(ctx, msgs.MsgParameterRequired, "lockId"), true, true}
	}
	if len(unlockParams.From) == 0 {
		return assembleOrEndorseError{i18n.NewError(ctx, msgs.MsgParameterRequired, "from"), true, true}
	}
	if len(unlockParams.Recipients) == 0 {
		return assembleOrEndorseError{i18n.NewError(ctx, msgs.MsgParameterRequired, "recipients"), true, true}
	}
	for _, entry := range unlockParams.Recipients {
		if entry.Amount == nil || entry.Amount.Int().Sign() != 1 {
			return assembleOrEndorseError{i18n.NewError(ctx, msgs.MsgParameterGreaterThanZero, "recipient amount"), true, true}
		}
	}
	return nil
}
...
func (h *unlockCommon) assembleUnlockOutputs_V0(ctx context.Context, tx *types.ParsedTransaction, params *types.UnlockParams, req *prototk.AssembleTransactionRequest, remainder *big.Int) (*preparedOutputs, *preparedLockedOutputs, AssembleError) {
	notary := tx.DomainConfig.NotaryLookup
...
) (*prototk.EndorseTransactionResponse, EndorseError) {
	if err := h.checkAllowed(ctx, tx, params.From); err != nil {
		return nil, err
	}
