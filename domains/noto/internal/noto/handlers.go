...
// response can carry a revert return AssembleError or EndorseError instead of error, so a
// handler cannot compile without saying whose fault each of its failures is. Init and Prepare
// have no revert in their proto responses, so they return a plain error and any classification
...
func (n *Noto) validateMintAmounts(ctx context.Context, params *types.MintParams, inputs, outputs *parsedCoins) EndorseError {
	if len(inputs.coins) > 0 {
		return endorseError{i18n.NewError(ctx, msgs.MsgInvalidInputs, "mint", inputs.coins), true}
	}
	if outputs.total.Cmp(params.Amount.Int()) != 0 {
		return endorseError{i18n.NewError(ctx, msgs.MsgInvalidAmount, "mint", params.Amount.Int().Text(10), outputs.total.Text(10)), true}
	}
	return nil
}
...
func (n *Noto) validateTransferAmounts(ctx context.Context, inputs, outputs *parsedCoins) EndorseError {
	if len(inputs.coins) == 0 {
		return endorseError{i18n.NewError(ctx, msgs.MsgInvalidInputs, "transfer", inputs.coins), true}
	}
	if inputs.total.Cmp(outputs.total) != 0 {
		return endorseError{i18n.NewError(ctx, msgs.MsgInvalidAmount, "transfer", inputs.total, outputs.total), true}
	}
	return nil
}
...
func (n *Noto) validateBurnAmounts(ctx context.Context, params *types.BurnParams, inputs, outputs *parsedCoins) EndorseError {
	if len(inputs.coins) == 0 {
		return endorseError{i18n.NewError(ctx, msgs.MsgInvalidInputs, "burn", inputs.coins), true}
	}
	amount := big.NewInt(0).Sub(inputs.total, outputs.total)
	if amount.Cmp(params.Amount.Int()) != 0 {
		return endorseError{i18n.NewError(ctx, msgs.MsgInvalidAmount, "burn", params.Amount.Int().Text(10), amount.Text(10)), true}
	}
	return nil
}
...
func (n *Noto) validateLockAmounts(ctx context.Context, tx *types.ParsedTransaction, inputs, outputs *parsedCoins) EndorseError {
	if tx.DomainConfig.IsV0() && len(inputs.coins) == 0 {
		...
		return endorseError{i18n.NewError(ctx, msgs.MsgInvalidInputs, "lock", inputs.coins), true}
	}
	amount := big.NewInt(0).Sub(inputs.total, outputs.total)
	if amount.Cmp(outputs.lockedTotal) != 0 {
		return endorseError{i18n.NewError(ctx, msgs.MsgInvalidAmount, "lock", outputs.lockedTotal.Text(10), amount.Text(10)), true}
	}
	return nil
}
...
func (n *Noto) validateUnlockAmounts(ctx context.Context, tx *types.ParsedTransaction, inputs, outputs *parsedCoins) EndorseError {
	if tx.DomainConfig.IsV0() && len(inputs.lockedCoins) == 0 {
		...
		return endorseError{i18n.NewError(ctx, msgs.MsgInvalidInputs, "unlock", inputs.lockedCoins), true}
	}
	amount := big.NewInt(0).Sub(inputs.lockedTotal, outputs.lockedTotal)
	if amount.Cmp(outputs.total) != 0 {
		return endorseError{i18n.NewError(ctx, msgs.MsgInvalidAmount, "unlock", outputs.total.Text(10), amount.Text(10)), true}
	}
	return nil
}
	...
	for _, states := range stateLists {
		for _, state := range states {
			...
			if existing, found := nullifiers[nullifier]; found {
				return endorseError{i18n.NewError(ctx, msgs.MsgDuplicateNullifierInList, existing, state.Id, nullifier), true}
			}
	...
	for _, states := range [][]*prototk.NewState{assembled.OutputStates, assembled.InfoStates} {
		for i, state := range states {
			...
			if len(state.NullifierSpecs) == 0 {
				return assembleError{i18n.NewError(ctx, msgs.MsgMissingNullifierSpec, i), false}
			}
			...
			for _, spec := range state.NullifierSpecs {
				if spec.PayloadType != expectedPayloadType {
					return assembleError{i18n.NewError(ctx, msgs.MsgNullifierWrongContract, i, expectedPayloadType, spec.PayloadType), false}
				}
			}
		}
	}
...
func (n *Noto) validateSignature(ctx context.Context, name string, attestations []*prototk.AttestationResult, encodedMessage []byte) EndorseError {
	...
	if signature == nil {
		return endorseError{i18n.NewError(ctx, msgs.MsgAttestationNotFound, name), true}
	}
	...
	if recoveredSignature.String() != signature.Verifier.Verifier {
		return endorseError{i18n.NewError(ctx, msgs.MsgSignatureDoesNotMatch, name, signature.Verifier.Verifier, recoveredSignature.String()), true}
	}
	return nil
}
...
func (n *Noto) validateOwners(ctx context.Context, owner string, verifiers []*prototk.ResolvedVerifier, coins []*types.NotoCoin, states []*prototk.StateRef) EndorseError {
	...
	for i, coin := range coins {
		if !coin.Owner.Equals(fromAddress.address) {
			return endorseError{i18n.NewError(ctx, msgs.MsgStateWrongOwner, states[i].Id, owner), true}
		}
	}
	return nil
}
...
// owner in each stage. On the originator this node resolved the list from the verifiers the
...
func (n *Noto) findEthAddressVerifier(ctx context.Context, errorDescription, lookup string, verifierList []*prototk.ResolvedVerifier) (*identityPair, AssembleOrEndorseError) {
	...
	if verifier == nil {
		return nil, assembleOrEndorseError{i18n.NewError(ctx, msgs.MsgErrorVerifyingAddress, errorDescription), false, true}
	}
	address, err := pldtypes.ParseEthAddress(verifier.Verifier)
	if err != nil {
		return nil, assembleOrEndorseError{err, false, true}
	}
