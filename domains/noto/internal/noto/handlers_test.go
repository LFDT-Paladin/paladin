...
func TestValidateMintAmountsRevertsOnInputs(t *testing.T) {
	...
	assertRevertAssemble(t, err, "PD200012.*mint")
	assertRevertEndorse(t, err, "PD200012.*mint")
}
...
func TestValidateMintAmountsRevertsOnMismatch(t *testing.T) {
	...
	assertRevertAssemble(t, err, "PD200013.*mint")
	assertRevertEndorse(t, err, "PD200013.*mint")
}
...
func TestValidateTransferAmountsRevertsOnNoInputs(t *testing.T) {
	...
	assertRevertAssemble(t, err, "PD200012.*transfer")
	assertRevertEndorse(t, err, "PD200012.*transfer")
}
...
func TestValidateTransferAmountsRevertsOnMismatch(t *testing.T) {
	...
	assertRevertAssemble(t, err, "PD200013.*transfer")
	assertRevertEndorse(t, err, "PD200013.*transfer")
}
...
func TestValidateBurnAmountsRevertsOnNoInputs(t *testing.T) {
	...
	assertRevertAssemble(t, err, "PD200012.*burn")
	assertRevertEndorse(t, err, "PD200012.*burn")
}
...
func TestValidateBurnAmountsRevertsOnMismatch(t *testing.T) {
	...
	assertRevertAssemble(t, err, "PD200013.*burn")
	assertRevertEndorse(t, err, "PD200013.*burn")
}
...
func TestValidateLockAmountsRevertsOnNoInputsInV0(t *testing.T) {
	...
	assertRevertAssemble(t, err, "PD200012.*lock")
	assertRevertEndorse(t, err, "PD200012.*lock")
}
...
func TestValidateLockAmountsRevertsOnMismatch(t *testing.T) {
	...
	assertRevertAssemble(t, err, "PD200013.*lock")
	assertRevertEndorse(t, err, "PD200013.*lock")
}
...
func TestValidateUnlockAmountsRevertsOnNoLockedInputsInV0(t *testing.T) {
	...
	assertRevertAssemble(t, err, "PD200012.*unlock")
	assertRevertEndorse(t, err, "PD200012.*unlock")
}
...
func TestValidateUnlockAmountsRevertsOnMismatch(t *testing.T) {
	...
	assertRevertAssemble(t, err, "PD200013.*unlock")
	assertRevertEndorse(t, err, "PD200013.*unlock")
}
...
func TestValidateOwnersRevertsOnWrongOwner(t *testing.T) {
	...
	assertRevertAssemble(t, n.validateOwners(t.Context(), "sender@node1", resolved, inputs.coins, inputs.states), "PD200018")
	assertRevertEndorse(t, n.validateOwners(t.Context(), "sender@node1", resolved, inputs.coins, inputs.states), "PD200018")

	locked := testLockedCoins(owner, 100)
	assertRevertAssemble(t, n.validateLockOwners(t.Context(), "sender@node1", resolved, locked.lockedCoins, locked.lockedStates), "PD200018")
	assertRevertEndorse(t, n.validateLockOwners(t.Context(), "sender@node1", resolved, locked.lockedCoins, locked.lockedStates), "PD200018")
}

func TestFindEthAddressVerifierRevertsAtEndorseButNotAssemble(t *testing.T) {
	n := &Noto{}

	_, err := n.findEthAddressVerifier(t.Context(), "sender", "sender@node1", nil)
	assertInternalAssemble(t, err, "PD200011.*'sender'")
	assertRevertEndorse(t, err, "PD200011.*'sender'")
	...
	assertInternalAssemble(t, err, "bad address")
	assertRevertEndorse(t, err, "bad address")
}

func signedMessage(t *testing.T, verifier string) ([]byte, []*prototk.AttestationResult) {
...
func TestValidateSignatureRevertsWhenAbsent(t *testing.T) {
	assertInternalAssemble(t, (&Noto{}).validateSignature(t.Context(), "sender", nil, []byte("the encoded transfer")), "PD200015.*'sender'")
	assertRevertEndorse(t, (&Noto{}).validateSignature(t.Context(), "sender", nil, []byte("the encoded transfer")), "PD200015.*'sender'")
}

func TestValidateSignatureRevertsWhenItWillNotRecover(t *testing.T) {
	message, attestations := signedMessage(t, "")
	attestations[0].Payload = []byte("not a signature")
	assertInternalAssemble(t, (&Noto{}).validateSignature(t.Context(), "sender", attestations, message), "FF22087")
	assertRevertEndorse(t, (&Noto{}).validateSignature(t.Context(), "sender", attestations, message), "FF22087")
}

func TestValidateSignatureRevertsWhenItRecoversToSomeoneElse(t *testing.T) {
	message, attestations := signedMessage(t, pldtypes.RandAddress().String())
	assertInternalAssemble(t, (&Noto{}).validateSignature(t.Context(), "sender", attestations, message), "PD200017.*'sender'")
	assertRevertEndorse(t, (&Noto{}).validateSignature(t.Context(), "sender", attestations, message), "PD200017.*'sender'")
}
