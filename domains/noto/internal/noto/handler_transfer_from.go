...
func (h *transferFromHandler) ValidateParams(ctx context.Context, config *types.NotoParsedConfig, params string) (any, AssembleOrEndorseError) {
	...
	if transferFromParams.From == "" {
		return &transferFromParams, assembleOrEndorseError{i18n.NewError(ctx, msgs.MsgParameterRequired, "from"), true, true}
	}
...
func (h *transferFromHandler) Assemble(ctx context.Context, tx *types.ParsedTransaction, req *prototk.AssembleTransactionRequest) (*prototk.AssembleTransactionResponse, AssembleError) {
	params := tx.Params.(*types.TransferFromParams)
	return h.assembleTransfer(ctx, tx, req, params.From, params.To, params.Amount, params.Data)
}

func (h *transferFromHandler) Endorse(ctx context.Context, tx *types.ParsedTransaction, req *prototk.EndorseTransactionRequest) (*prototk.EndorseTransactionResponse, EndorseError) {
	params := tx.Params.(*types.TransferFromParams)
