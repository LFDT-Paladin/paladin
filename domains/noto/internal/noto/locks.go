...
func (n *Noto) loadLockInfoV1(ctx context.Context, stateQueryContext string, lockID pldtypes.Bytes32) (*loadedLockInfo, AssembleError) {
	...
	if len(states) == 0 {
		return nil, assembleError{i18n.NewError(ctx, msgs.MsgLockIDNotFound), true}
	}
	...
	if err != nil {
		return nil, assembleError{i18n.WrapError(ctx, err, msgs.MsgInvalidLockState, lockState.Id), false}
	}
...
func (n *Noto) validateV1LockTransition(ctx context.Context, transitionType lockTransitionType, senderID *identityPair, lockID *pldtypes.Bytes32, inputs []*prototk.EndorsableState, outputs []*prototk.EndorsableState) (*lockTransition, EndorseError) {
	...
	case LOCK_CREATE:
		if len(inputLockInfoStates) != 0 || len(outputLockInfoStates) != 1 {
			return nil, endorseError{i18n.NewError(ctx, msgs.MsgInvalidLockTransition), true}
		}
	case LOCK_SPEND:
	...
	if len(inputLockInfoStates) == 1 {
		...
		if err != nil {
			return nil, endorseError{i18n.WrapError(ctx, err, msgs.MsgInvalidLockState, lt.prevLockState.Id), true}
		}
		...
		if senderID != nil && !lt.prevLockInfo.Owner.Equals(senderID.address) {
			return nil, endorseError{i18n.NewError(ctx, msgs.MsgStateWrongOwner, lt.prevLockState.Id, senderID.address), true}
		}
	}
	...
	if len(outputLockInfoStates) == 1 {
		...
		if err != nil {
			return nil, endorseError{i18n.WrapError(ctx, err, msgs.MsgInvalidLockState, lt.newLockState.Id), true}
		}
		...
		if lt.newLockInfo.Salt.IsZero() || lt.newLockInfo.Owner == nil || lt.newLockInfo.Spender == nil ||
			...
			return nil, endorseError{i18n.NewError(ctx, msgs.MsgInvalidLockState, lt.newLockState.Id), true}
		}
		// VAlidate the lockId if we were passed one
		if lockID != nil && !lt.newLockInfo.LockID.Equals(lockID) {
			return nil, endorseError{i18n.NewError(ctx, msgs.MsgInvalidLockStateLockID, lt.newLockState.Id, lockID, lt.newLockInfo.LockID), true}
		}
...
func (lt *lockTransition) splitOutputs(ctx context.Context, infoStates []*prototk.EndorsableState) (spendOutputs, cancelOutputs []*prototk.EndorsableState, err EndorseError) {
