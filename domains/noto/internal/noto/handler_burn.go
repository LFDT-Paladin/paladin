...
func (h *burnHandler) Assemble(ctx context.Context, tx *types.ParsedTransaction, req *prototk.AssembleTransactionRequest) (*prototk.AssembleTransactionResponse, AssembleError) {
	params := tx.Params.(*types.BurnParams)
	return h.assembleBurn(ctx, tx, req, tx.Transaction.From, params.Amount, params.Data)
}

func (h *burnHandler) Endorse(ctx context.Context, tx *types.ParsedTransaction, req *prototk.EndorseTransactionRequest) (*prototk.EndorseTransactionResponse, EndorseError) {
	params := tx.Params.(*types.BurnParams)
