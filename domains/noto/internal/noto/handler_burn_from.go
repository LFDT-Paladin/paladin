...
func (h *burnFromHandler) ValidateParams(ctx context.Context, config *types.NotoParsedConfig, params string) (any, AssembleOrEndorseError) {
	...
	if burnFromParams.From == "" {
		return &burnFromParams, assembleOrEndorseError{i18n.NewError(ctx, msgs.MsgParameterRequired, "from"), true, true}
	}
...
func (h *burnFromHandler) Assemble(ctx context.Context, tx *types.ParsedTransaction, req *prototk.AssembleTransactionRequest) (*prototk.AssembleTransactionResponse, AssembleError) {
	params := tx.Params.(*types.BurnFromParams)
	return h.assembleBurn(ctx, tx, req, params.From, params.Amount, params.Data)
}

func (h *burnFromHandler) Endorse(ctx context.Context, tx *types.ParsedTransaction, req *prototk.EndorseTransactionRequest) (*prototk.EndorseTransactionResponse, EndorseError) {
	params := tx.Params.(*types.BurnFromParams)
