...
// not satisfy the return type. Use internalErr, revert or endorseOnlyRevert to construct one.
// Each returns nil for a nil error, so they can be applied to an error that may or may not be
// set without producing a non-nil NotoDomainError holding a nil error.
//
...
// revert classifies a failure that the transaction is responsible for on every path: parameters
// that do not meet the contract's rules - e.g. amounts that do not balance, a signature that does
// not verify, states whose owner is not the party spending them. It reverts on both paths.
func revert(err error) NotoDomainError {
	if err == nil {
		return nil
	}
...
// endorseOnlyRevert classifies a failure where the culprit depends on the phase: at endorse the
// data being rejected is part of a transaction the originator assembled and proposed, so rejecting
// it is rejecting that proposal, but on the originator's own node the same data came from local
...
// assemble can prove invalid is still invalid at endorsement
func endorseOnlyRevert(err error) NotoDomainError {
	if err == nil {
		return nil
	}
