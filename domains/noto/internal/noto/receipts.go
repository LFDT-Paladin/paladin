	...
	if lockInfo != nil && len(receiptStates.ReadLockedInputs) > 0 && len(receiptStates.PreparedOutputs) > 0 {
		...
		if lockID != nil {
			...
			lockInfo.UnlockParams = map[string]any{
				...
				"lockedInputs":  n.endorsableStateIDs(ctx, n.filterSchema(req.ReadStates, []string{n.lockedCoinSchema.Id}), false),
				"lockedOutputs": n.endorsableStateIDs(ctx, n.filterSchema(req.InfoStates, []string{n.lockedCoinSchema.Id}), false),
				"outputs":       n.endorsableStateIDs(ctx, n.filterSchema(req.InfoStates, []string{n.coinSchema.Id}), false),
...
func (n *Noto) receiptLockInfoV1V2(ctx context.Context, req *prototk.BuildReceiptRequest, variant pldtypes.HexUint64) (lockInfo *types.ReceiptLockInfo, err error) {
	...
	if err == nil && !lt.newLockInfo.SpendTxId.IsZero() {
		...
		if lt.prevLockState == nil {
			...
			lockedInputIDs = n.endorsableStateIDs(ctx, n.filterSchema(req.OutputStates, []string{n.lockedCoinSchema.Id}), false)
		} else {
			// prepare unlock: locked coins are in ReadStates (they were created by a prior lock transaction)
			lockedInputIDs = n.endorsableStateIDs(ctx, n.filterSchema(req.ReadStates, []string{n.lockedCoinSchema.Id}), false)
		}
