...
func (h *unlockHandler) ValidateParams(ctx context.Context, config *types.NotoParsedConfig, params string) (any, NotoDomainError) {
...
func (h *unlockHandler) Assemble(ctx context.Context, tx *types.ParsedTransaction, req *prototk.AssembleTransactionRequest) (*prototk.AssembleTransactionResponse, NotoDomainError) {
	params := tx.Params.(*types.UnlockParams)
	...
	lockedInputs, err := h.noto.prepareLockedInputs(ctx, req.StateQueryContext, params.LockID, fromID.address, requiredTotal, true)
	if err != nil {
		return nil, err
	}
	...
	if tx.DomainConfig.IsV0() {
		outputs, v0LockedOutputs, err = h.assembleUnlockOutputs_V0(ctx, tx, params, req, remainder)
	} else {
...
func (h *unlockHandler) Endorse(ctx context.Context, tx *types.ParsedTransaction, req *prototk.EndorseTransactionRequest) (*prototk.EndorseTransactionResponse, NotoDomainError) {
	params := tx.Params.(*types.UnlockParams)
