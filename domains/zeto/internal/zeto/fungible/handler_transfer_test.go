...
func TestTransferPrepare(t *testing.T) {
	...
	tx.DomainConfig.ZetoVariant = pldtypes.HexUint64(types.ZetoFungibleABI_V1)
	res, err = h.Prepare(ctx, tx, req)
	assert.NoError(t, err)
	assert.Contains(t, res.Transaction.ParamsJson, "\"proof\":\"0x")
	assert.NotContains(t, res.Transaction.ParamsJson, "\"encryptionNonce\"")
	assert.Contains(t, res.Transaction.FunctionAbiJson, `"type":"bytes"`)
	tx.DomainConfig.ZetoVariant = pldtypes.HexUint64(types.ZetoFungibleABI_V0)
	...
	assert.NoError(t, err)
	assert.Contains(t, res.Transaction.ParamsJson, "\"proof\":\"0x")
	assert.NotContains(t, res.Transaction.ParamsJson, `"root":`)
	tx.DomainConfig.ZetoVariant = pldtypes.HexUint64(types.ZetoFungibleABI_V0)
}
...
func TestGenerateMerkleProofs(t *testing.T) {
	...
	smtName := zetosmt.MerkleTreeName("Zeto_Anon", addr)
	_, err = common.NewMerkleTreeSpec(ctx, smtName, smt.StatesTree, h.callbacks, h.stateSchemas.MerkleTreeRootSchema.Id, h.stateSchemas.MerkleTreeNodeSchema.Id, queryContext)
	assert.EqualError(t, err, "PD210019: Failed to create Merkle tree spec for smt_Zeto_Anon_0x1234567890123456789012345678901234567890: test error")
