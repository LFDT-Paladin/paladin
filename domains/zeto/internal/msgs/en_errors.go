...
var (
	...
	MsgUnsupportedZetoReleaseGeneration      = pde("PD210146", "Unsupported zeto release generation %d (deploy param \"factoryVersion\"; supported: 0, 1)")
	MsgZetoReleaseGenerationConflict         = pde("PD210147", "factoryVersion in deploy params (%d) conflicts with domain factoryVersion (%d)")
	...
	MsgDuplicateStateInList                  = pde("PD210156", "Duplicate state in list %s[%d] (%s)")
	MsgInvalidListInput                      = pde("PD210157", "Invalid item in list %s[%d] (%s): %s")
	MsgUnexpectedSchema                      = pde("PD210158", "Unexpected schema: %s")
	MsgUnsupportedZetoFungibleABIVersion     = pde("PD210159", "Unsupported zeto fungible ABI version %d (supported: 0, 1)")
)
